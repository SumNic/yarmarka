...
type AuthState = {
  ...
  isLoading: boolean;
  error: string | null;
  resendStatus: components["schemas"]["ResendConfirmationResponseDto"] | null;
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  changePassword: async (dto) => {
    ...
    try {
      ...
      set({
        isLoading: false,
        error: e instanceof Error ? e.message : "Ошибка смены пароля",
      });
    }
  },
  ...
  requestPasswordReset: async (dto) => {
    ...
    try {
      ...
      set({
        isLoading: false,
        error: e instanceof Error ? e.message : "Ошибка восстановления пароля",
      });
    }
  },
  ...
  resendConfirmation: async (email) => {
    const res = await api.auth.resendConfirmation({ email });
    set({ resendStatus: res });
  },
}));
